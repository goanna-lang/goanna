...
func TestTranspile(t *testing.T) {
	tests := []struct {
		...
		},
		{
			name: "full_example",
			src: func() string {
				b, _ := os.ReadFile(filepath.Join("..", "testdata", "full_example.goa"))
				return string(b)
			}(),
...
func TestTranspileFileNotFound(t *testing.T) {
	var buf bytes.Buffer
	err := TranspileFile("nonexistent_file_that_does_not_exist.goa", &buf)
	if err == nil {
...
func TestTranspileOutputIsValidGo(t *testing.T) {
	inputs, err := filepath.Glob(filepath.Join("..", "testdata", "*.goa"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestTranspileErrorFiles(t *testing.T) {
	inputs, err := filepath.Glob(filepath.Join("..", "testdata", "errors", "*.goa"))
	if err != nil {
		t.Fatal(err)
	}
	for _, input := range inputs {
...
func FuzzTranspile(f *testing.F) {
	...
	for _, p := range []string{
		"../testdata/gender_basic.goa",
		"../testdata/full_example.goa",
		"../testdata/payload_only.goa",
	} {
